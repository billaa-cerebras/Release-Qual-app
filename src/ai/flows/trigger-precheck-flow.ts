'use server';
/**
 * @fileOverview A flow for triggering Jenkins pre-check jobs for model releases.
 */
...
const triggerPrecheckJobsInputSchema = z.object({
  releases: z.array(modelReleaseSchema),
});

const triggerPrecheckJobsOutputSchema = z.object({
  success: z.boolean(),
  message: z.string(),
});
...
export const triggerPrecheckJobs = ai.defineFlow(
  {
    name: 'triggerPrecheckJobs',
    inputSchema: triggerPrecheckJobsInputSchema,
    outputSchema: triggerPrecheckJobsOutputSchema,
  },
  async ({ releases }) => {
    await dbConnect();
    ...
    if (!JENKINS_URL || !JENKINS_USERNAME || !JENKINS_API_TOKEN) {
      ...
      for (const release of releases) {
        await Job.create({
          releaseId: release.releaseTarget, modelName: release.modelName, type: 'PRECHECK',
          precheckStatus: 'FAILURE', precheckResult: message,
        });
      }
        ...
        try {
            ...
            if (response.status === 201) {
                ...
                await Job.create({
                    ...
                    type: 'PRECHECK', precheckStatus: 'PENDING', precheckResult: 'Pre-check job is queued.',
                });
            } else {
                ...
                if (errorText.includes('<!DOCTYPE html>')) {
                    ...
                    throw new Error(`Failed to trigger pre-check. Status: ${response.status}. Response: ${errorText}`);
                }
            }
        } catch (error: any) {
            allSuccessful = false;
            await Job.create({
                ...
                type: 'PRECHECK', precheckStatus: 'FAILURE', precheckResult: error.message || 'An unexpected error occurred.',
            });
        }
    }
    return { success: allSuccessful, message: allSuccessful ? 'All pre-check jobs triggered.' : 'Some pre-check jobs failed.' };
  }
);